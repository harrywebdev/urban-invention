...
const AccountBadge: FC<AccountBadgeProps> = ({ account, transferIcon }) => {
  ...
  const transferIconClass =
    transferIcon === "to" ? "pl-1" : transferIcon === "from" ? "pr-2" : "";

  return (
    <span
      className={`flex flex-row border rounded-md shadow-sm  ${getAccountColorClass(account.type)} ${transferIconClass} relative mx-2`}
    >
      <span className={"flex flex-col gap-1 px-3 py-2"}>
        <span
          className={
            "flex items-center text-neutral-900 dark:text-neutral-50 gap-1.5"
          }
        >
